...
const _generateAppCacheFile = () => {
    const appCacheFile = util.outdent`
        CACHE MANIFEST
        # ${new Date().getTime()}
        # This file is automatically generated
        CACHE:
        ${context.contextPath}/x/loader.gif
        ${context.contextPath}/x/scripts/x.js
        ${_cached.appcacheResources.map(r => `${context.contextPath}`).join('\n')}
        NETWORK:
        *
        FALLBACK:
        ${_.values(_cached.resourceInfoMap)
            .map(ri => `${ri.path}.html ${context.contextPath}${_cached.templateMap[_getTemplateKey(resInfo)]}`).join('\n')}
        /main.py /static.html
        images/large/ images/off`;
    logger.debug("Generated app cache:" + appCacheFile);
}
...
const _reloadTemplate = (resInfo) => _getTemplateData(resInfo.templateName.value)
    .then(data => {
        ...
        const fileName = `/__minimo_templates__${resInfo.templateName.orElse(resInfo.path)}.html`
