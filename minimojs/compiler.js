...
const compileResources = () =>
    components.startComponents().then(() =>
        resources.copy(_baseResPath, context.destinationPath)
...
const _generateAppCacheFile = () => resources.writeFile(`${context.destinationPath}/x/_appcache`, util.outdent`
    ...
    ${Array.from(_cached.appcacheResources).map(r => `${context.contextPath}${r}`).join('\n')}
    NETWORK:
    *
    FALLBACK:
    ${_.values(_cached.resourceInfoMap)
        .map(op => op.map(ri => `${ri.path}.html ${context.contextPath}${_cached.templateMap[_getTemplateKey(ri)]}`, ''))
        .join('\n')}`);
