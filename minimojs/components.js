...
const loadComponents => () {
    ...
    const filter = r => (r.endsWith(".js") || r.endsWith(".htmx")) && !r.match(/[^/]+$/)[0].startsWith(".")

    resources.getResources("./components", filter)
      .then(values =>
      ...
      .then(values =>
        [_compScripts, _compInfo, _htmxSourcesMap] = _loadComponents(values))
  }
}

module.exports = {
  //TODO clone
  getJsComponents: () => _compScripts

}

function _createComponentVO(resName, path, create, htmx) {
  return {
...
const _loadComponents = groupedResources => {
  ...
  Object.entries(groupedResources).forEach(entry => {
    ...
    if (!compName.startsWith("_")) {
      js = js.concat("components['", compName, "']=", varPath, ";");
      info.push(_createComponentVO(compName, varPath, hasHtmx));
    }
  });

  return [
    "var _comps = [" + info.map(c => "[\"" + c.resourceName + "\",\"" + c.varPath + "\"],").join()) + "];" +
